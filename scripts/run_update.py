...
def create_engines(get_engine: Callable[[str, str], Engine]) -> dict[str, Engine]:
    return {
        "sql02": get_engine("shinersql18", "NAV_LIVE"),
        "sql04": get_engine("shinersql04", "BC_LIVE_USA"),
        "sql05": get_engine("shinersql05", "BC_UAT_UK"),
        "finance": get_engine("shinersql18", "Finance"),
        "warehouse": get_engine("shinersql18", "data_control"),
    }
...
def main() -> int:
    ...
    engines = create_engines(shinerutils.get_sqlalchemy_engine)
    warehouse = engines["warehouse"]
    action = f"update [data_control].[dbo].[{args.table}] table"
    exit_code = 0

    try:
        update_function = getattr(shinerutils, definition.function_name)
