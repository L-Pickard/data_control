...
-- Rebuilds dbo.sales to position each persisted bin after its adjusted margin.
-- Holds a table write lock until commit. No Finance objects are changed.
-- Existing rows (including computed values), column types, indexes and FKs
-- are compared before commit. Any error restores the original table/data.
-- The table DDL below is a snapshot of sql/tables/sales.sql.
USE [data_control];
SET ANSI_NULLS ON;
...
BEGIN TRY
    ...
    IF COL_LENGTH('dbo.sales', 'gbp_margin_bin') IS NOT NULL
        THROW 51000, 'Margin bins already exist; this is a one-time migration.', 1;
    IF OBJECT_ID('dbo.sales_margin_bins') IS NOT NULL
        THROW 51000, 'The target view name is already in use; inspect before running.', 1;
    ...
    IF (SELECT COUNT(*) FROM sys.columns WHERE object_id=OBJECT_ID('dbo.sales')) <> 37
...
FROM sys.columns WHERE object_id=OBJECT_ID('dbo.sales')) AS metadata;
    -- Requested widening from nvarchar(2) to nvarchar(5); all other metadata must match.
    UPDATE #columns_before SET max_length=10
    WHERE name='doc_type' AND system_type_id=231 AND max_length=4;
...
WHERE f.parent_object_id=OBJECT_ID('dbo.sales')) AS metadata;
    ...
    EXEC sys.sp_executesql N'CREATE TABLE [dbo].[sales] (
        ...
        ,[gbp_margin_bin] AS (CONVERT(SMALLINT, CASE
        ...
        ,[eur_margin_bin] AS (CONVERT(SMALLINT, CASE
        ...
        ,[usd_margin_bin] AS (CONVERT(SMALLINT, CASE
...
-- Margin bins use adjusted margin / sales, with upper-inclusive 10-point bands.
-- -10 = <= -100%; -9..10 = (previous boundary, upper boundary];
-- 11 = > 100%; 127 = undefined (zero sales). The bin is also its sort key.
...
';
    ...
    IF EXISTS (SELECT [date_key], [date_key_ny], [posting_date], [document_date], [location_code], [customer_id], [document_no], [order_no], [doc_type], [salesperson_id], [country_id], [entity], [is_adjusted], [exclusion], [intercompany], [sales_type], [brand_id], [item_id], [quantity], [gbp_sales], [gbp_cost], [gbp_royalty], [gbp_rebate], [gbp_margin], [gbp_adjusted_margin], [eur_sales], [eur_cost], [eur_royalty], [eur_rebate], [eur_margin], [eur_adjusted_margin], [usd_sales], [usd_cost], [usd_royalty], [usd_rebate], [usd_margin], [usd_adjusted_margin] FROM dbo.sales EXCEPT SELECT [date_key], [date_key_ny], [posting_date], [document_date], [location_code], [customer_id], [document_no], [order_no], [doc_type], [salesperson_id], [country_id], [entity], [is_adjusted], [exclusion], [intercompany], [sales_type], [brand_id], [item_id], [quantity], [gbp_sales], [gbp_cost], [gbp_royalty], [gbp_rebate], [gbp_margin], [gbp_adjusted_margin], [eur_sales], [eur_cost], [eur_royalty], [eur_rebate], [eur_margin], [eur_adjusted_margin], [usd_sales], [usd_cost], [usd_royalty], [usd_rebate], [usd_margin], [usd_adjusted_margin] FROM #sales_before)
       OR EXISTS (SELECT [date_key], [date_key_ny], [posting_date], [document_date], [location_code], [customer_id], [document_no], [order_no], [doc_type], [salesperson_id], [country_id], [entity], [is_adjusted], [exclusion], [intercompany], [sales_type], [brand_id], [item_id], [quantity], [gbp_sales], [gbp_cost], [gbp_royalty], [gbp_rebate], [gbp_margin], [gbp_adjusted_margin], [eur_sales], [eur_cost], [eur_royalty], [eur_rebate], [eur_margin], [eur_adjusted_margin], [usd_sales], [usd_cost], [usd_royalty], [usd_rebate], [usd_margin], [usd_adjusted_margin] FROM #sales_before EXCEPT SELECT [date_key], [date_key_ny], [posting_date], [document_date], [location_code], [customer_id], [document_no], [order_no], [doc_type], [salesperson_id], [country_id], [entity], [is_adjusted], [exclusion], [intercompany], [sales_type], [brand_id], [item_id], [quantity], [gbp_sales], [gbp_cost], [gbp_royalty], [gbp_rebate], [gbp_margin], [gbp_adjusted_margin], [eur_sales], [eur_cost], [eur_royalty], [eur_rebate], [eur_margin], [eur_adjusted_margin], [usd_sales], [usd_cost], [usd_royalty], [usd_rebate], [usd_margin], [usd_adjusted_margin] FROM dbo.sales)
    ...
    is_nullable, ISNULL(collation_name,N'') AS collation_name, is_computed
FROM sys.columns WHERE object_id=OBJECT_ID('dbo.sales')) AS metadata;
    DELETE FROM #columns_after WHERE name IN ('gbp_margin_bin','eur_margin_bin','usd_margin_bin');
...
JOIN sys.partitions p ON p.object_id=i.object_id AND p.index_id=i.index_id
WHERE i.object_id=OBJECT_ID('dbo.sales')) AS metadata;
    -- The only intended index difference is three additional INCLUDE columns.
...
WHERE f.parent_object_id=OBJECT_ID('dbo.sales')) AS metadata;
    ...
    IF (SELECT COUNT(*) FROM sys.computed_columns
        WHERE object_id=OBJECT_ID('dbo.sales')
          ...
          AND COLUMNPROPERTY(object_id,name,'IsPrecise')=1) <> 3
        THROW 51000, 'Expected three precise, deterministic persisted SMALLINT bins.', 1;
    IF EXISTS (
        SELECT 1 FROM sys.columns b
        JOIN sys.columns m ON m.object_id=b.object_id AND m.name=LEFT(b.name,3)+'_adjusted_margin'
        WHERE b.object_id=OBJECT_ID('dbo.sales')
            AND b.name IN ('gbp_margin_bin','eur_margin_bin','usd_margin_bin')
            AND b.column_id<>m.column_id+1)
        THROW 51000, 'Margin bins are not beside their adjusted margins.', 1;

    EXEC sys.sp_executesql N'CREATE OR ALTER VIEW [dbo].[sales_margin_bins]
AS
-- One row per stored sales line; bin IDs are persisted and double as sort keys.
...
SELECT s.[date_key]
    ...
    ,CONVERT(VARCHAR(32), CASE
        WHEN s.[gbp_margin_bin] = 127 THEN ''Undefined / zero sales''
        WHEN s.[gbp_margin_bin] = -10 THEN ''<= -100%''
        WHEN s.[gbp_margin_bin] = 11 THEN ''> 100%''
        ELSE CONCAT(''> '', (s.[gbp_margin_bin] - 1) * 10, ''% & <= '', s.[gbp_margin_bin] * 10, ''%'')
    END) AS [gbp_margin_bin_label]
    ,s.[eur_sales]
    ,s.[eur_cost]
    ,s.[eur_royalty]
    ,s.[eur_rebate]
    ...
    ,s.[eur_margin_bin]
    ,CONVERT(VARCHAR(32), CASE
        WHEN s.[eur_margin_bin] = 127 THEN ''Undefined / zero sales''
        WHEN s.[eur_margin_bin] = -10 THEN ''<= -100%''
        WHEN s.[eur_margin_bin] = 11 THEN ''> 100%''
        ELSE CONCAT(''> '', (s.[eur_margin_bin] - 1) * 10, ''% & <= '', s.[eur_margin_bin] * 10, ''%'')
    END) AS [eur_margin_bin_label]
    ,s.[usd_sales]
    ,s.[usd_cost]
    ,s.[usd_royalty]
    ,s.[usd_rebate]
    ...
    ,s.[usd_margin_bin]
    ,CONVERT(VARCHAR(32), CASE
        WHEN s.[usd_margin_bin] = 127 THEN ''Undefined / zero sales''
        WHEN s.[usd_margin_bin] = -10 THEN ''<= -100%''
        WHEN s.[usd_margin_bin] = 11 THEN ''> 100%''
        ELSE CONCAT(''> '', (s.[usd_margin_bin] - 1) * 10, ''% & <= '', s.[usd_margin_bin] * 10, ''%'')
    END) AS [usd_margin_bin_label]
FROM [dbo].[sales] AS s
...
';
    ...
    EXEC sys.sp_executesql N'
        ...
        IF EXISTS (SELECT 1 FROM dbo.sales
           WHERE gbp_margin_bin IS NULL OR eur_margin_bin IS NULL OR usd_margin_bin IS NULL
              OR gbp_margin_bin NOT BETWEEN -10 AND 11 AND gbp_margin_bin<>127
              OR eur_margin_bin NOT BETWEEN -10 AND 11 AND eur_margin_bin<>127
              OR usd_margin_bin NOT BETWEEN -10 AND 11 AND usd_margin_bin<>127)
            THROW 51000, ''Invalid persisted bin.'', 1;';
    COMMIT TRANSACTION;
    SELECT 'PASS: sales rebuilt, data and metadata preserved, bins/index/view deployed' AS result,
