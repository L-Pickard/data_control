...
WITH external_sales AS (
    SELECT
        ...
        ,CASE
            WHEN s.[entity] IN (N'Shiner Ltd', N'Shiner B.V') THEN N'UK_EU'
            WHEN s.[entity] = N'Shiner LLC' THEN N'US'
    ...
    WHERE s.[intercompany] = 0
      AND s.[exclusion] = 0
      AND s.[entity] IN (N'Shiner Ltd', N'Shiner B.V', N'Shiner LLC')
)
SELECT
     [week_start]
    ,[planning_entity]
...
SELECT
     CASE
        WHEN [entity] IN (N'Shiner Ltd', N'Shiner B.V') THEN N'UK_EU'
        WHEN [entity] = N'Shiner LLC' THEN N'US'
        ELSE N'UNMAPPED'
     END AS [planning_entity]
