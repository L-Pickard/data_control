WITH purchase_headers AS (
	SELECT 'Shiner Ltd' AS [entity]
		...
		,ph.[Deposit Payment Date], ph.[Balance Payment Date], ph.[Shiner Ref_]
		,ph.[Intercompany Doc No_], ph.[3PL Sent], ph.[3PL Date Time]
		,ph.[Last Modified Date Time]
	FROM [Shiner$Purchase Header] AS ph
	...
	SELECT 'Shiner B.V' AS [entity]
		...
		,ph.[Intercompany Doc No_], ph.[3PL Sent], ph.[3PL Date Time]
		,ph.[Last Modified Date Time]
	FROM [Shiner BV$Purchase Header] AS ph
	WHERE ph.[Document Type] = 1
),
purchase_lines AS (
	SELECT 'Shiner Ltd' AS [entity]
	...
	FROM [Shiner$Purchase Line] AS pl
	...
	SELECT 'Shiner B.V' AS [entity]
		...
		,pl.[Outstanding Amount (LCY)]
	FROM [Shiner BV$Purchase Line] AS pl
	WHERE pl.[Document Type] = 1
)
...
SELECT pl.[entity]
	...
	,NULLIF(ph.[Shiner Ref_], '') AS [shiner_reference]
