WITH inventory_components AS (
	...
	FROM [Shiner$Item Ledger Entry] AS ile
	LEFT JOIN [Shiner$Item] AS i ON i.[No_] = ile.[Item No_]
	...
	SELECT N'Shiner B.V'
		...
		,CAST(0 AS DECIMAL(38, 20))
	FROM [Shiner BV$Item Ledger Entry] AS ile
	LEFT JOIN [Shiner BV$Item] AS i ON i.[No_] = ile.[Item No_]
	WHERE ile.[Location Code] = N'3PL'
		AND LEN(ile.[Item No_]) <= 20
	GROUP BY ile.[Item No_]
), reservations AS (
	SELECT N'Shiner Ltd' AS [entity], [Item No_] AS [item_id], SUM([Quantity]) AS [reserved_quantity]
	FROM [Shiner$Reservation Entry]
	WHERE [Location Code] = N'BRISTOL' AND [Source Type] = 32
		AND [Source Subtype] = 0 AND [Reservation Status] = 0
	GROUP BY [Item No_]
	UNION ALL
	SELECT N'Shiner B.V', [Item No_], SUM([Quantity])
	FROM [Shiner BV$Reservation Entry]
	WHERE [Location Code] = N'3PL' AND [Source Type] = 32
		AND [Source Subtype] = 0 AND [Reservation Status] = 0
	GROUP BY [Item No_]
), unavailable AS (
	SELECT N'Shiner Ltd' AS [entity], [Item No_] AS [item_id], SUM([Quantity]) AS [unavailable_quantity]
	FROM [Shiner$Warehouse Entry]
