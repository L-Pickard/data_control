...
FROM [shiner$Item] AS it

FULL OUTER JOIN [shiner BV$Item] AS bt
	ON it.[No_] = bt.[No_]

LEFT JOIN [Shiner$Vendor] AS vn
	ON ISNULL(it.[Vendor No_], bt.[Vendor No_]) = vn.[No_]

LEFT JOIN [Shiner$Item Category] AS ct
	ON it.[Item Category Code] = ct.[Code]

LEFT JOIN [Shiner$Product Group] AS pg
