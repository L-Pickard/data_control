WITH sales_headers AS (
	SELECT N'Shiner Ltd' AS [entity], h.[Document Type], h.[No_], h.[Sell-to Customer No_],
		...
		h.[Intercompany Doc No_], h.[Shiner Ref_], h.[Last Modified Date Time],
		h.[Pre Order], h.[Order Status]
	FROM [Shiner$Sales Header] AS h
	WHERE h.[Document Type] = 1

	UNION ALL

	SELECT N'Shiner B.V' AS [entity], h.[Document Type], h.[No_], h.[Sell-to Customer No_],
		h.[Bill-to Customer No_], h.[Your Reference], h.[Ship-to Country_Region Code],
		h.[Ship-to Code], h.[Document Date], h.[Order Date], h.[Posting Date],
		h.[Payment Terms Code], h.[Due Date], h.[Shipment Method Code],
		h.[Customer Posting Group], h.[Currency Code], h.[Currency Factor],
		...
		h.[Intercompany Doc No_], h.[Shiner Ref_], h.[Last Modified Date Time],
		h.[Pre Order], h.[Order Status]
	FROM [Shiner BV$Sales Header] AS h
	WHERE h.[Document Type] = 1
),
sales_lines AS (
	SELECT N'Shiner Ltd' AS [entity], l.[Document Type], l.[Document No_], l.[Line No_],
	...
	FROM [Shiner$Sales Line] AS l
	WHERE l.[Document Type] = 1

	UNION ALL

	SELECT N'Shiner B.V' AS [entity], l.[Document Type], l.[Document No_], l.[Line No_],
		l.[Type], l.[No_], l.[Shipment Date], l.[Location Code],
		l.[Shortcut Dimension 1 Code], l.[Shortcut Dimension 2 Code],
		l.[Customer Price Group], l.[Purchase Order No_], l.[Purch_ Order Line No_],
		l.[Drop Shipment], l.[Quantity], l.[Outstanding Quantity], l.[Unit Price],
		...
		l.[Amount], l.[Amount Including VAT], l.[Outstanding Amount],
		l.[Quantity Shipped], l.[Quantity Invoiced]
	FROM [Shiner BV$Sales Line] AS l
	WHERE l.[Document Type] = 1
)
SELECT l.[entity]
	...
	,COALESCE(NULLIF(h.[Currency Code], N''), CASE l.[entity] WHEN N'Shiner B.V' THEN N'EUR' ELSE N'GBP' END) AS [currency_code]
	...
	,NULLIF(h.[Shiner Ref_], N'') AS [shiner_reference]
