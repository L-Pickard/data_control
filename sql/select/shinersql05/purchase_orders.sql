WITH purchase_headers AS (
	SELECT 'Shiner Ltd' AS [entity]
		...
		,phe.[Shiner Ref_$7345c412-e77c-4377-90eb-8798f2bfcc1c] AS [extension_shiner_reference]
	...
	FROM [Shiner UAT UK$Purchase Header$437dbf0e-84ff-417a-965d-ed2bb9650972] AS ph

	LEFT JOIN [Shiner UAT UK$Purchase Header$437dbf0e-84ff-417a-965d-ed2bb9650972$ext] AS phe
	...
	SELECT 'Shiner B.V' AS [entity]
		...
		,phe.[Shiner Ref_$7345c412-e77c-4377-90eb-8798f2bfcc1c]
	...
	FROM [Shiner UAT BV$Purchase Header$437dbf0e-84ff-417a-965d-ed2bb9650972] AS ph

	LEFT JOIN [Shiner UAT BV$Purchase Header$437dbf0e-84ff-417a-965d-ed2bb9650972$ext] AS phe
		ON phe.[Document Type] = ph.[Document Type]
		AND phe.[No_] = ph.[No_]
...
purchase_lines AS (
	SELECT 'Shiner Ltd' AS [entity], pl.*
	FROM [Shiner UAT UK$Purchase Line$437dbf0e-84ff-417a-965d-ed2bb9650972] AS pl
	WHERE pl.[Document Type] = 1

	UNION ALL

	SELECT 'Shiner B.V' AS [entity], pl.*
	FROM [Shiner UAT BV$Purchase Line$437dbf0e-84ff-417a-965d-ed2bb9650972] AS pl
	WHERE pl.[Document Type] = 1
)
...
SELECT pl.[entity]
	...
	,COALESCE(NULLIF(ph.[Currency Code], ''), CASE pl.[entity] WHEN 'Shiner B.V' THEN 'EUR' ELSE 'GBP' END) AS [currency_code]
	...
	,NULLIF(ph.[extension_shiner_reference], '') AS [shiner_reference]
